...
#ID        RA          Dec           X            Y           mag         dmag            FWHM  Flags" > new_sources.html
...
if [ -s fading_sources.txt ]; then
    ...
    for id in `awk '{if($1<0) print $1}' fading_sources.txt`; do
        grep -v '#' stack_${cam}_radec.txt | awk '{if($15=='$id') print $0,"fading_source"}'
    done > tmp$$.txt
    cat stack_${cam}_radec.txt tmp$$.txt | grep -v '#' | sort -ur -k 3 | awk '{if($1!=l) print; l=$1}' | sort -n -k 3 >> stack_${cam}_radec.tmp
    rm tmp$$.txt
else
    cp stack_${cam}_radec.txt stack_${cam}_radec.tmp
fi
...
grep -v '#' stack_${cam}_radec.tmp | awk '{if (system("[ -f lc_"$15".jpg ]")) printf("%d\n",$15); else printf("<A HREF=\"lc_%d.jpg\" TARGET=\"_blank\">%d</A>\n",$15,$15)}' > tag_radec.txt
grep -v '#' stack_${cam}_radec.tmp | awk '{s=""; if(NF>15) {for(i=16;i<=NF;i++) s=s" "$i}; printf("%-12.6f %-12.6f %-12.6f %-12.6f %-12.6f %-12.6f %8.2f %s\n",$1,$2,$8,$9,$3,$4,$7,s)}' > rest_radec.txt
...
#ID        RA          Dec           X            Y           mag         dmag            FWHM  Flags" > catalogued_sources.html

grep '#' stack_${cam}_radec0.txt > stack_${cam}_radec0.tmp
grep -v '#' stack_${cam}_radec0.txt | awk '{if($15<0) print $0,"usno_high_pm"; else print}' >> stack_${cam}_radec0.tmp
...
grep -v '#' stack_${cam}_radec0.tmp | awk '{if (system("[ -f lc_"$15".jpg ]")) printf("%d\n",$15); else printf("<A HREF=\"lc_%d.jpg\" TARGET=\"_blank\">%d</A>\n",$15,$15)}' > tag_radec0.txt
grep -v '#' stack_${cam}_radec0.tmp | awk '{s=""; if(NF>15) {for(i=16;i<=NF;i++) s=s" "$i}; printf("%-12.6f %-12.6f %-12.6f %-12.6f %-12.6f %-12.6f %8.2f %s\n",$1,$2,$8,$9,$3,$4,$7,s)}' > rest_radec0.txt
...
if [ "$pos_err0" ]; then
    ...
    cdec0=`echo $dec0 | awk '{print cos('$dec0'/57.296)}'`
    ...
    #grep -v '#' index.tmp1 | awk '{if($2~/HREF/) {ra=$4;dec=$5} else {ra=$2;dec=$3}; dx=(ra-'$ra0')*'$cdec0'; dy=dec-('$dec0'); dis=sqrt(dx*dx+dy*dy)*3600.; if (dis<'$pos_err1') printf("%s %8.1f arcsec from center\n",$0,dis)}' | sed -e "s/lc_/${pdir0}\/lc_/g" >> index.html
    grep -v '#' index.tmp1 | awk '{if($2~/HREF/) {x=$6;y=$7} else {x=$4;y=$5}; dx=x-'$sx0'; dy=y-'$sy0'; dis=sqrt(dx*dx+dy*dy)*'$ps'; if (dis<'$pos_err1') printf("%s %8.1f arcsec from center\n",$0,dis)}' | sed -e "s/lc_/${pdir0}\/lc_/g" >> index.html
